...
const smtp_relay = nodemailer.createTransport({
  host: "",
  port: 0,
  auth: {
    user: "",
...
app.get("/article/:title", (req, res) => {
  ...
  fs.readFile(
    path.join(__dirname, `articles/${req.params.title}.txt`),
    { encoding: "utf-8" },
    function (err, data) {
      if (!err) {
        res.render("pages/article", {
          ...
          text: data,
          css: `/styles/${req.params.title}.css`,
          js: `/js/${req.params.title}.js`,
        });
      } else {
        console.log(err);
      }
...
app.get("/tool/:title", (req, res) => {
  console.log(req.params.title);
  fs.readFile(
    path.join(__dirname, `tools/${req.params.title}.html`),
    { encoding: "utf-8" },
    function (err, data) {
      if (!err) {
        fs.readFile(
          path.join(__dirname, `tools/metadata/${req.params.title}.json`),
          { encoding: "utf-8" },
          function (err_meta, data_meta) {
            if (!err_meta) {
...
app.post("/guest-submit", (req, res) => {
  ...
  if (typeof req.body.textMessage === "string") {
    infoBody.textMessage = req.body.textMessage.replace("<", "&lt;").replace(">", "&gt;");
  }

  if (typeof req.body.username === "string") {
    infoBody.username = req.body.username.replace("<", "&lt;").replace(">", "&gt;");
  }
...
app.get("/test/:stff", (req, res) => {
  console.log(req.socket.remoteAddress, req.headers["x-forwarded-for"], req.ip);
  res.redirect(`https://${req.params.stff}/`);
});
...
app.get("/break-me", (req, res) => {
  ...
  if (sess == undefined || sess_maps.get(sess) == undefined) {
    let sess = createHash("sha256")
      .update(req.ip + Math.random().toString())
      .digest("hex");
    //
...
app.get("/demon", (req, res) => {
  ...
  if (
    sess == undefined ||
    sess_maps.get(sess) == undefined ||
    key == undefined ||
    sess_maps.get(sess).key != key
  ) {
  ...
  socket.on("message", (message) => {
    ...
    if (jsonmsg.id == undefined || jsonmsg.sess == undefined || !sess_maps.has(jsonmsg.sess)) {
      ...
      socket.send(
        JSON.stringify({
          id: 0,
          key: sess_maps.get(jsonmsg.sess).key,
          st: sess_maps.get(jsonmsg.sess).stt,
        })
      );
